...
export const getUserPostReactions = async (
  ...
  if (!reactionsStr) {
    return handleError(
      `cannot find reactions with id ${reactionsID}`,
      request,
      [],
      HTTPStatus.NOT_FOUND,
    )
  }
  const reactions = plainToClass(IReactions, JSON.parse(reactionsStr))
...
export const react = async (request: IttyRequest): Promise<Response> => {
  ...
  if (!postRes) {
    return handleError(
      `no post found with id ${args.post}`,
      request,
      [],
      HTTPStatus.NOT_FOUND,
    )
  }
  const postObj = plainToClass(IPost, JSON.parse(postRes))

  const postReaction = postObj.reactions.find(
  ...
  if (!postReaction) {
    ...
    if (!reactionStr) {
      ...
      const reactions = plainToClass(IReactions, JSON.parse(reactionStr))
