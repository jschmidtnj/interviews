...
func GetKeys() error {
	...
	privateKeyPassword := js.Global().Get("PRIVATE_KEY_PASSWORD").String()
	...
	// }
	res, err := jwt.ParseRSAPrivateKeyFromPEMWithPassword(privatePem, privateKeyPassword)
	if err != nil {
		return err
	}
	privateKey = *res
	pubKey, err := jwt.ParseRSAPublicKeyFromPEM(publicPem)
	if err != nil {
		return err
	}
